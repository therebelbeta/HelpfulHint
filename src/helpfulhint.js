...
var BabyHint = {
  ...
  removeEndOfMultilineComment: _removeEndOfMultilineComment,

  removeStrings: _removeStrings,

  checkFunctionDecl: _checkFunctionDecl,

  checkBannedProperties: _checkBannedProperties,

  checkSpelling: _checkSpelling,

  editDistance: _editDistance,

  checkSpaceAfterVar: _checkSpaceAfterVar,

  checkTrailingEquals: _checkTrailingEquals,

  getVariables: _getVariables,

  checkFunctionParams: _checkFunctionParams
}
...
function _removeEndOfMultilineComment (line) {
  var index = line.indexOf('*/')
  if (index !== -1) {
    BabyHint.inComment = false
...
function _checkFunctionDecl (line, lineNumber) {
  var errors = []
  ...
  _.each(functions, function (fun) {
    ...
    var error = {
      ...
      text: interpolate("If you want to define a function, you should use 'var %(name)%s = function() {}; ' instead!", {
        name: name
      }, intdelim),
      breaksCode: true,
...
function _checkBannedProperties (line, lineNumber) {
  var errors = []
  ...
  _.each(words, function (word) {
    if (BabyHint.bannedProperties.hasOwnProperty(word)) {
      var error = {
        ...
        text: interpolate('%(word)%s is a reserved word.', {
          word: word
        }, intdelim),
        breaksCode: true,
        source: 'bannedwords'
...
function _checkSpelling (line, lineNumber) {
  var errors = []
  var words = line.split(/[^~`@#\$\^\w]/g)
  var skipNext = false
  _.each(words, function (word) {
    if (word.length > 0 && !skipNext) {
      ...
      if (dist > 0 &&
        ...
        var error = {
          row: lineNumber,
          column: line.indexOf(word),
          text: interpolate("Did you mean to type '%(keyword)%s' instead of '%(word)%s'?", {
            keyword: keyword,
            word: word
          }, intdelim),
          breaksCode: false,
        ...
        if (BabyHint.functionFormSuggestion[keyword]) {
          error.text += ' ' + interpolate("In case you forgot, you can use it like '%(usage)%s'", {
...
function _editDistance (word) {
  ...
  _.each(BabyHint.keywords.concat(BabyHint.variables), function (keyword) {
    ...
    for (var c = 1; c < cols; c++) {
      diff = keyword[0] === word[c] ? 0 : 1
      table[0][c] = table[0][c - 1] + diff
    }
    // initialize first col
    for (var r = 1; r < rows; r++) {
      diff = keyword[r] === word[0] ? 0 : 1
...
function _checkSpaceAfterVar (line, lineNumber) {
  ...
  if (matches) {
    ...
    var error = {
      ...
      text: interpolate("Did you forget a space between 'var' and '%(variable)%s'?", {
...
function _checkTrailingEquals (line, lineNumber) {
  var errors = []
  var i = line.length - 1
  ...
  if (line[i] === '=') {
    var error = {
      row: lineNumber,
      column: i,
      text: "You can't end a line with '='",
      breaksCode: true
    }
    errors.push(error)
  }
...
function _checkFunctionParams (line, lineNumber) {
    ...
    if (line[i] === '(') {
      ...
      if (stack.length === 0) {
        error = {
          row: lineNumber,
          column: i,
          text: "It looks like you have an extra ')'",
          breaksCode: false,
  ...
  if (stack.length > 0) {
    ...
    error = {
      row: lineNumber,
      column: stack.pop(),
      text: "It looks like you are missing a ')' - does every '(' have a corresponding closing ')'?",
      breaksCode: false,
      source: 'paramschecker'
    }
    errors.push(error)
  ...
  for (i = functions.length - 1; i >= 0; i--) {
    ...
    if (spacesBetween) {
      ...
      error = {
        row: lineNumber,
        column: col,
        text: 'Did you forget to add a comma between two parameters?',
    ...
    if (!_.include(objectFunctions, functions[i])) {
      ...
      if (typeof expectedParams !== 'undefined') {
        ...
        if (typeof expectedParams === 'number' &&
          ...
          var msg = expectedParams > 1 ? '%(name)%s takes %(num)%s parameters, not %(given)%s!' : '%(name)%s takes 1 parameter, not %(given)%s!'
          var obj = expectedParams > 1 ? {
            name: functionCall,
            given: numParams,
            num: expectedParams
          } : {
          ...
          text = interpolate(msg, obj, intdelim)
          ...
          text = interpolate('%(name)%s takes %(list)%s parameters, not %(given)%s!', {
            name: functionCall,
            list: listOfParams,
            given: numParams
          }, intdelim)
      ...
      if (text) {
        ...
        if (functionForm) {
          text = interpolate("It looks like you're trying to use %(name)%s. In case you forgot, you can use it like: %(usage)%s", {
            name: functionCall,
            usage: "'" + functionForm + "'"
          }, intdelim)
        }

...
module.exports = function _helpfulHint(opts){
  debug('helpfulhint vinyl plugin')
  return through({ objectMode: true }, 
    _hhBuffer, // transform is a noop
    _hhEnd
  )
}
function _hhBuffer(chunk, enc, cb){
  debug('success',chunk.jshint.success)
  debug('errorCount',chunk.jshint.errorCount)
  debug('results',chunk.jshint.results)
  debug('data',chunk.jshint.data)
  debug('opt',chunk.jshint.opt)

  var program = chunk.contents.toString().split('\n')
  console.log(program)
  if (chunk.isNull()) {
    this.push(chunk);
    cb(null, chunk);
